...
async def test_create_kostenstelle(client, session: AsyncSession):
    resp = await client.post("/api/finanzen/kostenstellen", json={
        "name": "Fußball",
...
async def test_list_kostenstellen(client, session: AsyncSession):
    ...
    # Create one
    await client.post("/api/finanzen/kostenstellen", json={
        "name": "Tennis",
        "budget": 5000.00,
    })
...
async def test_update_kostenstelle(client, session: AsyncSession):
    ...
    resp2 = await client.put(f"/api/finanzen/kostenstellen/{ks_id}", json={
        "budget": 5000.00,
...
async def test_update_kostenstelle_not_found(client):
    resp = await client.put("/api/finanzen/kostenstellen/9999", json={
        "budget": 1000.00,
    })
...
async def test_leistungsverrechnung(client, session: AsyncSession):
    ...
    resp1 = await client.post("/api/finanzen/kostenstellen", json={
        "name": "Fußball",
        "budget": 10000.00,
    })
    ks1_id = resp1.json()["id"]

    resp2 = await client.post("/api/finanzen/kostenstellen", json={
        "name": "Tennis",
        "budget": 5000.00,
    })
    ...
    booking_resp = await client.post("/api/finanzen/buchungen", json={
    ...
    resp = await client.post("/api/finanzen/leistungsverrechnung", json={
...
async def test_leistungsverrechnung_invalid_sum(client, session: AsyncSession):
    ...
    ks_resp = await client.post("/api/finanzen/kostenstellen", json={
        "name": "Test KS",
        "budget": 5000.00,
    })
